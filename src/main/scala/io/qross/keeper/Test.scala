...
object Test {
    def main(args: Array[String]): Unit = {
        ...
        */
        ...
        dh.executeNonQuery("ALTER TABLE qross_jobs_denpendencies MODIFY COLUMN id INT")
        ...
        dh.open("mysql.qross_release").saveAs("mysql.qross")
            ...
            .get("SELECT id, job_id, upstream_ids, title, command_type, command_text, overtime, retry_limit, create_time, update_time FROM qross_jobs_dags WHERE job_id>1")
                .put("INSERT INTO qross_jobs_dags (id, job_id, upstream_ids, title, command_type, command_text, overtime, retry_limit, create_time, update_time FROM qross_jobs_dags) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            ...
            .get("SELECT id, job_id, task_id, upsteam_ids, command_id, status, retry_times, start_time, run_time, finish_time, elapsed, waiting, create_time, update_time FROM qross_tasks_dags WHERE job_id>1")
                .put("INSERT INTO qross_tasks_dags (id, job_id, task_id, upsteam_ids, command_id, status, retry_times, start_time, run_time, finish_time, elapsed, waiting, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
        ...
        dh.executeNonQuery("ALTER TABLE qross_jobs_denpendencies MODIFY COLUMN id INT AUTO_INCREMENT")
        ...
        dh.executeNonQuery("ALTER TABLE qross_tasks_logs MODIFY COLUMN id BIGINT AUTO_INCREMENT")
